...
module Spec.Homework.One where
...
traceValidGrabTT = do
  ...
  Emulator.callEndpoint @"grab" h4 (True, True)
...
traceValidGrabFF = do
  (_, _, _, h4, _) <- baseTrace
  Emulator.callEndpoint @"grab" h4 (False, False)
  _ <- Emulator.waitNSlots 1
...
traceInvalid = do
  (_, _, _, h4, _) <- baseTrace
  Emulator.callEndpoint @"grab" h4 (False, True)
  _ <- Emulator.waitNSlots 1

  Emulator.callEndpoint @"grab" h4 (True, False)
  _ <- Emulator.waitNSlots 1

...
mkValidator beneficiary timeOfWithdrawal _ ctx = 
    traceIfFalse "invalid beneficiary" (txSignedBy info beneficiary) &&
    traceIfFalse "time to withdraw funds not yet reached" (from timeOfWithdrawal `contains` txInfoValidRange info)
    where
        info :: TxInfo
